                ...
                <div class="bg-white relative shadow-md sm:rounded-lg overflow-hidden">
                    <div
                        ...
                        <div class="w-full md:w-64">
                            <form method="GET" action="{{ route('admin.mahasiswa.index') }}" class="flex items-center">
                        ...
                        <table class="w-full text-sm text-left text-gray-600">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                                <tr>
                                    ...
                                    <th scope="col" class="px-4 py-3">Enrollment Date</th>
                            ...
                            <tbody>
                                @foreach ($students as $student)
                                    <tr class="border-b border-gray-200">
                                        ...
                                        <td class="px-4 py-3">{{ $student->graduation_date ?? 'N/A' }}</td>
                                        <td class="px-4 py-3">{{ $student->enrollment_date }}</td>
                                        <td class="px-4 py-3">{{ $student->status }}</td>
                                        <td class="px-4 py-3">{{ $student->User_id }}</td>
                                        <td class="px-4 py-3">{{ $student->email }}</td>
                                        <td class="px-4 py-3">
                                            <button data-modal-target="student-modal-{{ $student->id }}"
                                                data-modal-toggle="student-modal-{{ $student->id }}" type="button"
